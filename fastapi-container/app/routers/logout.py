...
from app.routers.login import User, get_current_active_user, oauth2_scheme
...
async def log_someone_out(
    ...
    upd_rec = JWT_REVOCATION_COLLECTION.find_one_and_update(
        {"username": current_user.username}, {"$set": {"jwt": token[37:]}}
    )
    if upd_rec is None:
        JWT_REVOCATION_COLLECTION.insert_one(
            {"username": current_user.username, "jwt": token[37:]}
        )
    return {"status": "ok"}
