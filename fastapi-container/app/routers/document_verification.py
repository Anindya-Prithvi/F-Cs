...
async def verify_upload_file(file: UploadFile, signature: str, current_user: Annotated[User, Depends(get_current_active_user)]):
    # file.
    # verify! 
    
    if str.isnumeric(signature) == False:
        raise HTTPException(status_code=422, detail="signature not numeric")

    if __verify_file_blob(file=file, signature=signature, user=current_user) == False:
...
async def download_file(current_user: Annotated[User, Depends(get_current_active_user)]):
    encoded_file = PROPERTY_DOCUMENTS_COLLECTION.find_one({})
    file = Binary(encoded_file["encoded_file"])
    
    return StreamingResponse(content=iter_bytes(encoded_file["encoded_file"]), media_type=encoded_file["file_headers"]["content-type"], headers={"filename": encoded_file["file_name"]})
